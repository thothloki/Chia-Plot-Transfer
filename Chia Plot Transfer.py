...
#DRIVE THAT YOUR PLOT FILES ARE PLOTTED TO
tempDrive = r'C:\path\to\plots'
#DRIVE(S) THAT YOU ARE TRANSFERRING TO FOR MINING. SEPARATE EACH DRIVE WITH A COMMA. MAKE SURE YOU HAVE AN r TO INDICATE A RAW FILE LOCATION
farmDrives = [r'E:', r'F:'] 
...
def main():
    #GET LIST OF PLOTS
    plots = [each for each in os.listdir(tempDrive) if each.endswith(fileFormat)]

    for drive in farmDrives:
        for plot in plots:
            _, _, free = shutil.disk_usage(drive)
            if free > os.path.getsize(os.path.join(tempDrive, plot)):
                print(free)
                print(os.path.getsize(os.path.join(tempDrive, plot)))
                plotdir = os.path.join(tempDrive, plot)
